subroutine gcp(n,iz,xyz,echo,method,ebsse,g)
implicit none
integer n,maxat,max_elem,max_para
...
real*8 va,vb
real*8 r,dx,dy,dz
real*8 damp,tmp,ea,ecp,dum,tmpa,tmpb,dum2,autokcal
...
!     if(grad.and.echo)  call prtim(6,tg,'t','gradient')
      ...
      if (echo) then
        write(*,*)''
        write(*,'('' E_gCP /kcal,au:'',f11.4,f12.8)') ebsse*autokcal,ebsse
      end if      
      end
...
data BASsvp/2*5,6,9,6*14,2*10,6*18,14,24,10*31,6*32/   
