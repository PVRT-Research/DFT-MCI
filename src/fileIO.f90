!!------------ Load input file
subroutine readiog(input)
use defvar
use util
implicit real*8 (a-h,o-z)
character c80tmp*80,input*80

open(20,file=input)
call loclabel(20,'exchange',ifound)
if (ifound==1) then 
	read(20,*) c80tmp,ksnamex
	write(*,"(' KS exchange: ',A)") trim(ksnamex)
end if 
call loclabel(20,'correlation',ifound)	 
if (ifound==1) then
	read(20,*) c80tmp,ksnamec
	write(*,"(' KS correlation: ',A)") trim(ksnamec)
end if
! doK not implemented yet, we need an interface to an integral code
!call loclabel(20,'doK',ifound)
!if (ifound==1) then
!	read(20,*) c80tmp,doK
!	if (doK==1) write(*,"(' doing J, K and HF exchange')")
!end if
call loclabel(20,'doINT',ifound)
if (ifound==1) then
	read(20,*) c80tmp,doINT
end if
call loclabel(20,'MClvl',ifound)
if (ifound==1) then
	read(20,*) c80tmp,MClvl
end if

call loclabel(20,'boxscale',ifound)
if (ifound==1) then
	read(20,*) c80tmp,boxscale
end if


call loclabel(20,'MCprc',ifound)
if (ifound==1) then
	read(20,*) c80tmp,epsabs
end if
call loclabel(20,'doJ',ifound)
if (ifound==1) then
	read(20,*) c80tmp,doJ
	if (doJ==1.and.doK==0) write(*,"(' doing J')")
end if
call loclabel(20,'grid',ifound)
if (ifound==1) then
	read(20,*) c80tmp,radpot,sphpot
	write(*,"(' radial and angular points of the integration grid:',2I4)") radpot,sphpot
end if
call loclabel(20,'doGrimme',ifound)
if (ifound==1) then
	read(20,*) c80tmp,doGrimme
	if (doK==1) write(*,"(' doing Grimme corrections')")
end if


!Not implemented yet:
!
!call loclabel(20,'print')
...
call loclabel(20,'density',ifound)
if (ifound==1) then 
	read(20,*) c80tmp,filename
	write(*,"(' name of density file: ',A)") trim(filename)
	write(*,*)
	call readinfile(filename,0)	  
else 
	write(*,"(a)") " +--> ERROR: at least a file containing the density is requred <--+"
	stop
end if
...
if (thisfilename(inamelen-2:inamelen)=="iog".or.thisfilename(inamelen-2:inamelen)=="IOG") then
	write(*,"(a)") " ---=< Reading iog input file >=---"
	call readiog(thisfilename)
else if (thisfilename(inamelen-2:inamelen)=="fch".or.thisfilename(inamelen-3:inamelen)=="fchk"&
...
.or.thisfilename(inamelen-3:inamelen)=="FCHK") then
	write(*,"(' ---=< structural information in ',a,' file >=---')") trim(thisfilename)
...
if (infomode==0) write(*,"(' The highest angular momentum basis functions is ',a)") shtype2name(imaxL) 
if (imaxL>=2) then
	rewind(10)
	do while(.true.)
		read(10,"(a)") c80
...
!!!------ Output molecular formula and coordinates
...
do iatm=1,ncenter
    write(*,"(' 'a,3f20.10)") trim(ind2name(a(iatm)%index)),a(iatm)%x*b2a,a(iatm)%y*b2a,a(iatm)%z*b2a
end do
write(*,*)
end subroutine
